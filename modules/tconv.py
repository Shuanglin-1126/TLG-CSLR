...
class TemporalConv(nn.Module):
    def __init__(self, input_size, hidden_size, conv_type=2, use_bn=False, num_classes=-1):
        ...
        for layer_idx, ks in enumerate(self.kernel_size):
            ...
            if ks[0] == 'P':
                ...
                #  padding=int(ks[1]) // 2
                modules.append(
                    nn.Conv1d(input_sz, self.hidden_size, kernel_size=int(ks[1]), stride=1)
    ...
    def forward(self, frame_feat, lgt):
        ...
        # return visual_feat.permute(2, 0, 1), lgt.cpu()
        logits = None if self.num_classes == -1 \
            else self.fc(visual_feat.transpose(1, 2)).transpose(1, 2)
        return visual_feat.permute(2, 0, 1), lgt.cpu(), logits.permute(2, 0, 1)
        # return {
