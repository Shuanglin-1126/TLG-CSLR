...
#             return x
#         keer_prob = 1 - self.drop_prob
#         shape = (x.shape[0],) + (1,) + (x.ndim)
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, checkpointing=False, drop_path=0.):
        super().__init__()
    ...
    def attention(self, x: torch.Tensor):
        ...
        #
        return self.attn(x, torch.concat((x, self.prefix_embedding_k.repeat(1, x.shape[1], 1)), 0),
                         torch.concat((x, self.prefix_embedding_v.repeat(1, x.shape[1], 1)), 0), need_weights=True,
    ...
    def forward(self, x: torch.Tensor):
        if self.checkpointing:
            ...
            x = residual + checkpoint(self.attention, x, use_reentrant=False)[0] + self.S_Adapter(residual)
        else:
            residual = x
            x = self.ln_1(x)
            x = residual + self.attention(x)[0] + self.S_Adapter(residual)
        # x = x + self.S_Adapter(self.attention(self.ln_1(x)))
        # x = x + self.mlp(self.ln_2(x))

        if self.checkpointing:
            residual = x
            x = checkpoint(self.ln_2, x, use_reentrant=False)
            x = residual + checkpoint(self.mlp, x, use_reentrant=False) + self.MLP_Adapter(residual)
        else:
            residual = x
            x = self.ln_2(x)
            x = residual + self.mlp(x) + self.MLP_Adapter(residual)
...
class Correlation_Module(nn.Module):
    ...
    def forward(self, x, upfold):
        ...
        features = torch.einsum('lon,ond->lnd', affinities, upfold)
...
class TemporalAggregationBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor, T):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        x = self.linear(x)
        x_upfold = self.upfold(x[..., self.d_model:].permute(1, 2, 0), T).permute(2, 0, 1)  # LND -> NDL -> LND
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, checkpointing=True):
        ...
        self.checkpointing = checkpointing
        self.layers = layers
        drop_path = 0.1 if layers == 12 else 0.3
        if self.checkpointing:
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock(width, heads, attn_mask, checkpointing=i >= 0, drop_path=drop_path) for i in range(layers)])
        else:
            self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask, drop_path=drop_path) for _ in range(layers)])
    ...
    def forward(self, x: torch.Tensor, T=None):
        ...
        for i in range(len(self.resblocks)):
            x, attn = checkpoint(self.resblocks[i], x, use_reentrant=False) if self.checkpointing else self.resblocks[i](x)
            attn_list.append(attn)
            # if self.checkpointing:
            #     query = checkpoint(self.aggblocks[i], x, query, use_reentrant=False)
            # else:
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, idx_patches):
        ...
        x, attn_list = self.transformer(x, T)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, idx_patches):
        return self.visual(image.type(self.dtype), idx_patches)
